/**
 * Simple Bible API Library
 * Uses actual Bible text with minimal sacred names mapping
 */

import completeBibleData from '../data/bible/complete-bible-original-kjv.json';
...
// Simple sacred names mappings (only core ones)
...
// Convert complete Bible data to our format
function convertBibleData(): Book[] {
  const books: Book[] = [];
  
  console.log('Converting complete Bible data...', completeBibleData.length, 'books found');
  
  for (let i = 0; i < completeBibleData.length; i++) {
    const bookData = completeBibleData[i] as BibleBookData;
...
export function getBooks(group?: string): Book[] {
  if (!cachedBooks) {
    console.log('Loading complete Bible data for the first time...');
