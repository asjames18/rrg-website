/**
 * Complete Bible API Library
 * Uses a public domain Bible API to fetch real Bible text
 */
...
// Load sample Bible data from JSON file
import completeBibleData from '../data/bible/sample-bible.json';

// Convert the loaded data to our format
const SAMPLE_BIBLE_DATA = {};

// Process the complete Bible data
completeBibleData.forEach((book: any) => {
  SAMPLE_BIBLE_DATA[book.book] = {
    chapters: {}
  };
  
  book.chapters.forEach((chapter: string[], chapterIndex: number) => {
    SAMPLE_BIBLE_DATA[book.book].chapters[chapterIndex + 1] = chapter;
  });
});
...
function convertBibleData(): Book[] {
  ...
  console.log('Converting Bible data with actual verses...');
