 ...
 * Uses actual Bible text from sample data
 */

import sampleBibleData from '../data/bible/kjv.sample.json';
...
// Clean sacred names mappings
const SACRED_NAMES: Record<string, string> = {
  // Core Divine Names (order matters - most specific first)
...
function convertBibleData(): Book[] {
  ...
  console.log('Converting sample Bible data...', sampleBibleData.length, 'books found');
  
  for (let i = 0; i < sampleBibleData.length; i++) {
    const bookData = sampleBibleData[i] as BibleBookData;
...
export function getBooks(group?: string): Book[] {
  if (!cachedBooks) {
    console.log('Loading sample Bible data for the first time...');
